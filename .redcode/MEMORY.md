...
- **Android 全链路已真机打通**：订阅导入（Rust HTTPS）→ 72 节点进内核 → TUN 接管（tun0）→ 其他 App 流量走代理节点（`www.google.com -> [Hy2]Taiwan9` 实测，哥哥平板浏览器亲测成功）
- **Android 移植已提交未 push**：d749d028（feat 全链路，17 文件）+ 36d11f84（fmt 收尾）；三件套回归全绿（typecheck / cargo test / aarch64 cargo check）
- **待办（按序）**：
  - Android 端 UI 适配：去掉桌面装饰件（顶部红黄绿三灯在触屏无用）、布局按平板触屏走——用户 260906 提出，界面不能照抄桌面
  - push（待哥哥点头）
  - 桌面内核升级：删 sidecar 或 prebuild `--force` 可刷到 v1.19.30
