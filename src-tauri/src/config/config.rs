    ...
    pub async fn generate_file(typ: ConfigType) -> Result<PathBuf> {
            ...
            .unwrap_or(false)
        {
            ...
            let tun_fd = vpn
                .start_vpn()
                .await
                .map_err(|error| anyhow!("failed to start Android VPN: {error}"))?;
            set_tun_file_descriptor(&mut config, tun_fd)?;
        }
...
fn set_tun_file_descriptor(config: &mut Mapping, tun_fd: i32) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn test_set_tun_file_descriptor() {
        let mut config = Mapping::new();
        config.insert("tun".into(), Value::Mapping(Mapping::new()));
