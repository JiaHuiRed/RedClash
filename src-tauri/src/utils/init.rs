...
async fn initialize_config_files() -> Result<()> {
        ...
        && !path.exists()
    {
        ...
        help::save_yaml(&path, &template, Some("# RedClash"))
            .await
...
pub fn init_scheme() -> Result<()> {
    ...
    clash.set_value("", &"RedClash")?;
    clash.set_value("URL Protocol", &"RedClash URL Scheme Protocol")?;
...
#[cfg(target_os = "linux")]
pub fn init_scheme() -> Result<()> {
    const DESKTOP_FILE: &str = "red-clash.desktop";
...
#[cfg(target_os = "linux")]
const DEEP_LINK_SCHEMES: &[&str] = &["clash", "red-clash"];
